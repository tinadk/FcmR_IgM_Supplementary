#!/usr/bin/env python3
"""
Figure_2 : Occurrence of residue pairs across 7 structures (core/semi-core/peripheral).
...
plt.rcParams['font.size'] = 9    # all text = 9pt
...
def main():
    ...
    # Use same margins as Fig3 (left, right, top, bottom) - adjust bottom for xlabel
    fig.subplots_adjust(left=0.10, right=0.88, top=0.94, bottom=0.12)
    ...
    # Save with bbox_inches='tight' to keep labels within
    base_name = OUTPUT_DIR / 'Figure_2_occurrence_barplot'
    for ext in ('png', 'jpg', 'pdf'):
        fig.savefig(f"{base_name}.{ext}", dpi=DPI, facecolor='white', bbox_inches='tight')
    plt.close(fig)
    print(f"Figures saved to {OUTPUT_DIR}")

    df_out = pd.DataFrame({'pair': labels, 'occurrence': counts})
    csv_path = OUTPUT_DIR / 'Figure_2_occurrence_full_residue_barplot.csv'
    df_out.to_csv(csv_path, index=False)
    print(f"CSV saved → {csv_path}")

    xlsx_path = OUTPUT_DIR / 'Figure_2_occurrence_barplot.xlsx'
    try:
        # Rename the first column to the desired descriptive name
        df_out.rename(columns={'pair': 'FcμR-D1–IgM-Cμ4 Residue Pair'}, inplace=True)
