#!/usr/bin/env python3
"""
Fig. 8B – Free energy landscapes (FEL) overlay (colored by system) and 2x2.
2x2 colormap: plasma (purple‑yellow, no black), contour lines white (thin, semi‑transparent).
Overlay: colored by system (same as Fig.8A) for comparability.
...
OUTPUT_DIR = OUT_DIR / 'fig8b_fels_output'
...
if all_rows:
    combined_csv = OUTPUT_DIR / 'Fig8B_overlay_data.csv'
    ...
    xlsx_path = OUTPUT_DIR / 'Fig8B_FEL_data.xlsx'
...
print('1. Plotting FEL scatter overlay (colored by system)...')
...
ax.set_title('Free energy landscape (all systems)')
...
for ext in ('png', 'jpg', 'pdf'):
    plt.savefig(OUTPUT_DIR / f'Fig8B_FEL_overlay.{ext}', dpi=DPI,
                bbox_inches='tight', facecolor='white')
plt.close()
print('  Overlay saved.')

# ===================== 2x2 FEL (plasma + white contours) =====================
print('2. Plotting FEL 2x2 (plasma, white contours)...')
...
plt.tight_layout()
for ext in ('png', 'jpg', 'pdf'):
    plt.savefig(OUTPUT_DIR / f'Fig8B_FEL_2x2.{ext}', dpi=DPI,
                bbox_inches='tight', facecolor='white')
plt.close()
print('  2x2 saved.')

print(f'All Fig.8B outputs in {OUTPUT_DIR}')