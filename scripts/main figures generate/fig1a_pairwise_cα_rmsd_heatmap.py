#!/usr/bin/env python3
"""
Fig. 1A – Cα RMSD heatmap among FcμR structures.
...
OUTPUT_DIR = OUT_DIR / 'fig1a_rmsd_heatmap'
...
# ----- Match colorbar height to heatmap height -----
cbar = ax.collections[0].colorbar
cax = cbar.ax
# Get the heatmap axis position
pos_ax = ax.get_position()
# Get the current colorbar position and adjust its height
pos_cax = cax.get_position()
cax.set_position([pos_cax.x0, pos_ax.y0, pos_cax.width, pos_ax.height])

for ext in ('png', 'jpg', 'pdf'):
    plt.savefig(OUTPUT_DIR / f'fig1a_rmsd_heatmap.{ext}', dpi=DPI, bbox_inches='tight')
plt.close()
print(f"Heatmap saved to {OUTPUT_DIR}")

# ---------- Export data table ----------
df_rmsd = pd.DataFrame(rmsd_matrix, index=labels, columns=labels)
csv_path = OUTPUT_DIR / 'rmsd_matrix.csv'
df_rmsd.to_csv(csv_path, float_format='%.2f')
print(f"CSV saved to {csv_path}")

xlsx_path = OUTPUT_DIR / 'rmsd_matrix.xlsx'
